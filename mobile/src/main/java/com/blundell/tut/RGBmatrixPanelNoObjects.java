...
class RGBmatrixPanelNoObjects {
     ...
     * Max 7
     */
    private static final int PWM_BITS = 1;
     ...
     * 00-111-111
     */
    private byte[][][] displayRowColumn = new byte[PWM_BITS][ROWS_PER_SUB_PANEL][COLUMN_COUNT];
//    private byte[][][] fadeInDisplayRowColumn = new byte[PWM_BITS][ROWS_PER_SUB_PANEL][COLUMN_COUNT]; //2nd plane for handling fadeIn
...
//            timingLogger.addSplit("loop row " + row);
            ...
            for (int d = 0; d < PWM_BITS; d++) {
//                timingLogger.addSplit("loop pwm " + d);
                byte[] rowData = displayRowColumn[d][row];
...
//                    timingLogger.addSplit("loop col " + col);
                    byte colPins = rowData[col];

                    // Clear bits (clock)
//                    gpioProxy.writeClock(false);
//                  sleep(stabilizeWait);

                    gpioProxy.writePixelsSequence(colPins);
//                    sleep(stabilizeWait);
...
//                timingLogger.addSplit("done loop pwm " + d);
            }
